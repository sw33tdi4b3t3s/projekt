...
router.post('/add', async (req, res) => {
    try {
        ...
        const countryHorse = await Country.findOne({ code: countryCode.trim().toUpperCase() });
        if (!countryHorse) {
            return res.status(404).json({ error: `Nie znaleziono kraju o kodzie ISO: ${countryCode}` });
        }

        const countryBreeder = await Country.findOne({ code: breederCountryCode.trim().toUpperCase() });
        if (!countryBreeder) {
            return res.status(404).json({ error: `Nie znaleziono kraju hodowcy o kodzie ISO: ${breederCountryCode}` });
        }
        ...
        if (!breederDoc) {
            return res.status(404).json({ 
                error: `Nie znaleziono hodowcy "${breederName}" zarejestrowanego w kraju ${breederCountryCode.toUpperCase()}` 
            });
        }

        let fatherDoc = null;
        if (father && father.name && father.countryCode && father.birthYear) {
           const fatherCountry = await Country.findOne({code: father.countryCode.trim().toUpperCase()});
           if(!fatherCountry){
                return res.status(404).json({error: `nie znaleziono kraju ojca konia o kodzie: ${father.countryCode}`});
           }
           ...
           if(fatherDoc.gender !== 'ogier'){
            return res.status(400).json({error: `ojciec musi byc ogierem, a jest: ${fatherDoc.gender}`});
           }
        }

        let motherDoc = null;
        if (mother && mother.name && mother.countryCode && mother.birthYear) {
            const motherCountry = await Country.findOne({code: mother.countryCode.trim().toUpperCase()});
            if(!motherCountry){
                return res.status(404).json({error: `nie znaleziono kraju matki konia o kodzie: ${mother.countryCode}`});
            }
            ...
            if(motherDoc.gender !== 'klacz'){
                return res.status(400).json({error: `matka musi byc klaczą, a jest: ${motherDoc.gender}`});
            }
        }
...
router.route('/:name/:country/:birthYear')
    ...
    try {
        ...
        if (countryCode) {
            ...
            if (!country) {
                return res.status(404).json({ error: `Nie znaleziono nowego kraju o kodzie ISO: ${countryCode}` });
            }
        ...
        if (breederName !== undefined) {
            if (breederName === null || breederName.trim() === "") {
                ...
                if (breederCountryCode) {
                    const breederCountry = await Country.findOne({ code: breederCountryCode.toUpperCase() });
                    if (!breederCountry) {
                        return res.status(404).json({ error: `Nie znaleziono kraju hodowcy o kodzie ISO: ${breederCountryCode}` });
                    }
        ...
        if (father !== undefined) {
            if (father === null) {
                ...
                if (father.countryCode) {
                    const fatherCountry = await Country.findOne({ code: father.countryCode.toUpperCase() });
                    if (!fatherCountry) {
                        return res.status(404).json({ error: `Nie znaleziono kraju ojca o kodzie ISO: ${father.countryCode}` });
                    }
        ...
        if (mother !== undefined) {
            if (mother === null) {
                ...
                if (mother.countryCode) {
                    const motherCountry = await Country.findOne({ code: mother.countryCode.toUpperCase() });
                    if (!motherCountry) {
                        return res.status(404).json({ error: `Nie znaleziono kraju matki o kodzie ISO: ${mother.countryCode}` });
                    }
